...
# Utility functions
def save_to_csv(result_dict, output_file):
...
def predict(test_set, pipeline, temperature, dataset_name, min_length, top_k, top_p, batch_size=20):
    ...
    for batch in tqdm(dataloader, total=len(dataloader)):
        ...
        for i in range(len(batch['context'])):
            message = f"Here is the Query: {batch['query'][i]}. Here is the context: . Answer:"
            #message = f"Here is the Query: {batch['query'][i]}. Here is the context: {batch['context'][i]}. Answer:"
            #message = [{"role": "user", "content": f"Here is the Query: {batch['query'][i]}, here is the Article: {batch['context'][i]}"}]
        ...
        #contents = [output[0]["generated_text"][-1]["content"].strip() for output in outputs]
...
if __name__ == "__main__":
    ...
    if dataset_name == "PubMedQA":
        ...
        raw_test_set = load_dataset(dataset_name, split="test[:10]", trust_remote_code=True)
    elif dataset_name == 'cnn':
    ...
    file_name = f'zeroshot-{dataset_name}_{model_id.split("/")[-1]}.csv'
    df.to_csv(os.path.join(dir_name, file_name))

    evaluator = Evaluator()
    file_name = f'evaluation-zeroshot-{dataset_name}_{model_id.split("/")[-1]}.csv'
