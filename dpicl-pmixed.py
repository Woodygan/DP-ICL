...
def predict(test_set, ensemble, tokenizer, temperature, dataset_name, min_length, top_k, top_p,tokenwise=True, endtoend=True, split=1,n_length=128):
    predictions = []
    for idx, row in tqdm(enumerate(test_set), total=len(test_set)):       
        output=ensemble.priv_generate(row['context'], n_length, row['query'], temperature=temperature, max_length=max_new_tokens, top_k=None,tokenwise=tokenwise, endtoend=endtoend,split=split)
...
if __name__ == "__main__":
    ...
    ensemble_model = PMixED(base_model, model_id, tokenizer, 
                            device=device, q_budget=30, alpha=2, delta=1e-5,
                            p=0.03, eps=50000, beta=0., lambd=None, 
    ...
    predictions = predict(test_set, ensemble_model, tokenizer, temperature=temp, dataset_name=dataset_name, min_length=min_new_tokens,top_k=top_k,top_p=top_p,endtoend=False,n_length=n_length)
    ...
    file_name = f'pmixed-{dataset_name}-{testsize}-{model_id.split("/")[-1]}-noEndtoEnd.csv'
    df.to_csv(os.path.join(dir_name, file_name))

    evaluator = Evaluator()
    file_name = f'evaluation-pmixed-{dataset_name}_{model_id.split("/")[-1]}.csv'
